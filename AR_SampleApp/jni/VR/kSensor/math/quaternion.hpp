...
struct QuaternionT {
    ...
    void ToMatrix4x4(GVR::mat4 &m) ;
    T Dot(const QuaternionT<T>& q) const;
    void ToMatrix4x4(T *m) ;
    ...
    QuaternionT<T> operator+(const QuaternionT<T>& q) const;
    QuaternionT<T> operator*(const QuaternionT<T>& q) const;
    QuaternionT<T> operator/(T val) const;
    ...
    Vector3<T> Rotate_(const Vector3<T>& v) const;
...
template <typename T>
inline QuaternionT<T>::QuaternionT(T w, Vector3<T> v) : x(v.x), y(v.y), z(v.z), w(w)
{
}
...
template <typename T>
inline QuaternionT<T> QuaternionT<T>::Inverse() const
{
	T dotprod = (x * x + y * y + z * z + w * w);
	return Inverted()/dotprod;
}
...
template <typename T>
	inline void QuaternionT<T>::ToMatrix4x4(GVR::mat4& mat)
{
    const T s = 2;
    T xs, ys, zs;
    T wx, wy, wz;
    ...
    m[1] = xy + wz; 		m[5] = 1 - (xx + zz); 	m[9] = yz - wx;
		...
		mat.x.x = 1 - (yy + zz);     mat.x.y = xy+ wz; 			mat.x.z = xz - wy;
		mat.y.x = xy - wz;             mat.y.y =  1.0f - (xx+zz); 		mat.y.z = yz + wx;
		mat.z.x = xz + wy;		mat.z.y = yz - wx;			mat.z.z = 1.0f - (xx + yy);
...
template <typename T>
QuaternionT<T> QuaternionT<T>::operator/(T val) const
{
	
	return QuaternionT<T>(x/val,y/val,z/val,w/val);
}
...
template <typename T>
inline Vector3<T> QuaternionT<T>::Rotate_(const Vector3<T>& v) const
{
		Vector3<T> uv, uuv;
		Vector3<T> QuatVector(x, y, z);
...
inline QuaternionT<T> QuaternionT<T>::CreateFromEulerAngle(
	...
	QuaternionT<T> qAroundX = QuaternionT<T>::CreateFromAxisAngle(Vector3<T>(1.0f, 0.0f, 0.0f), (euler_angle.x));
	QuaternionT<T> qAroundY = QuaternionT<T>::CreateFromAxisAngle(Vector3<T>(0.0f, 1.0f, 0.0f), (euler_angle.y));
	QuaternionT<T> qAroundZ = QuaternionT<T>::CreateFromAxisAngle(Vector3<T>(0.0f, 0.0f, 1.0f), (euler_angle.z));
	Quaternion qRes = qAroundZ.Multiplied(qAroundY).Multiplied(qAroundX);//1108
